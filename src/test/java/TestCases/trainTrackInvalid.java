...
public class trainTrackInvalid extends base {
	...
	public void initialize() throws Exception {
		...
		String runMode=reader.getCellData(11, 1);
		if(runMode.equalsIgnoreCase("yes")) {
			driver = initializeDriver();
			driver.get(prop.getProperty("url"));
		}
		else if(runMode.equalsIgnoreCase("no")) {
			reader.setCellData(testExecution, "Skipped", 11, 2);
	...
	public void trainTracking() throws Exception {
		...
		ExcelUtil reader = new ExcelUtil();
		reader.setExcelFile(Path,"trainTrackInvalid");
		...
		reader1.setCellData(testExecution, "Pass", 11, 2);
