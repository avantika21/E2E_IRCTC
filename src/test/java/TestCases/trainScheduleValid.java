...
public class trainScheduleValid extends base{
	...
	public void initialize() throws Exception {
		...
		String runMode=reader.getCellData(9, 1);
		if(runMode.equalsIgnoreCase("yes")) {
			driver = initializeDriver();
			driver.get(prop.getProperty("url"));
		}
		else if(runMode.equalsIgnoreCase("no")) {
			reader.setCellData(testExecution, "Skipped", 9, 2);
	...
	public void trainSchedule() throws Exception {
		...
		ExcelUtil reader = new ExcelUtil();
		reader.setExcelFile(Path,"trainScheduleValid");
		...
		reader1.setCellData(testExecution, "Pass", 9, 2);
