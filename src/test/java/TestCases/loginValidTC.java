...
public class loginValidTC extends base{
	...
	public void initialize() throws Exception {
		...
		String runMode=reader.getCellData(5, 1);
		if(runMode.equalsIgnoreCase("yes")) {
			driver = initializeDriver();
			driver.get(prop.getProperty("url"));
		}
		else if(runMode.equalsIgnoreCase("no")) {
			reader.setCellData(testExecution, "Skipped", 5, 2);
	...
	public void basePageNavigation() throws Exception {
		...
		ExcelUtil reader = new ExcelUtil();
		reader.setExcelFile(Path,"loginValid");
		...
		reader1.setCellData(testExecution, "Pass", 5, 2);
